...
public interface CollectService {
    ...
    Observable<ResponseBean<String>> collect(@Path("id") int id);

    /* 收藏（站外文章） */
    @FormUrlEncoded
    @POST("lg/collect/add/json") // title，author，link
    Observable<ResponseBean<String>> collect(@Field("title") String title, @Field("author") String author, @Field("link") String link);

    /* 取消收藏 （文章列表）*/
    @POST("lg/uncollect_originId/{id}/json")
    Observable<ResponseBean<String>> unCollect(@Path("id")int id);

    /* 取消收藏（我的收藏页面）*/
    @FormUrlEncoded
    @POST("lg/uncollect/{id}/json") // originId 代表的是收藏之前的那篇文章本身的id,没有为-1
    Observable<ResponseBean<String>> unCollect(@Path("id") int id, @Field("originId") int originId);
